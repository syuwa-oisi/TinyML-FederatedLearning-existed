...
def print_until_keyword(keyword, arduino):
    print("print_until_keyword sleep start")
    time.sleep(1)
    print("print_until_keyword sleep end")
    while True:
        try:
            print("print_until_keyword")
            msg = arduino.readline()
            print(msg)
            print(msg.decode())
            msg = msg.decode().replace('<', '')
        except UnicodeDecodeError:
            print("print_until_keyword exeption")
            msg = arduino.readline()
            print(msg)
            print(msg.decode())
            msg = msg.decode()
...
#     while True:
#         while (pauseListen):
#             print(f"Paused listening on device {device.port}")
#             time.sleep(0.1)
...
#             print(f"Serial exception on device {device.port}: {e}")
#             # 再接続処理
#             try:
#                 with serial_lock:
#                     device.close()
#                     device.open()
#                     print(f"Reconnected to device {device.port}")
#             except Exception as reconnect_error:
#                 print(f"Failed to reconnect to device {device.port}: {reconnect_error}")
#                 break  # 再接続に失敗した場合はループを抜ける

def listenDevice(device, deviceIndex, device_lock):
    global pauseListen, graph
    while True:
        print("while loop")
        while pauseListen:
...
def FlGetModel(d, device_index, devices_hidden_layer, devices_output_layer, devices_num_epochs, old_devices_connected):
    global size_hidden_layer, size_output_layer, pauseListen,devices_connected
    pauseListen = True
    ...
    if read == b'<': # Python <-- SYN ACK <-- Arduino
        print("write s")
...
def sendModel(d, hidden_layer, output_layer):
    ini_time = time.time()
    print(f'Starting connection to {d.port} ...')
    for i in range(size_hidden_layer): # hidden layer
...
listen_threads = []
device_locks = [threading.Lock() for _ in devices]

for i, (d, device_lock) in enumerate(zip(devices, device_locks)):
    print(f"Starting thread for device {d.port}")
    thread = threading.Thread(target=listenDevice, args=(d, i, device_lock))
    thread.daemon = True
    thread.start()
