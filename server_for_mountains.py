...
batch_size = 25 # Must be even, hsa to be split into 2 types of samples
...
random.shuffle(test_montserrat_files)
random.shuffle(test_pedraforca_files)
random.shuffle(test_vermell_files)
...
def sendSamplesIID(device, deviceIndex, batch_size, batch_index):
    ...
    for i, filename in enumerate(files):
        ...
        sendSample(device, f'datasets/{"colors" if num_button == 3 else "mountains"}/{filename}', num_button, deviceIndex)
...
def sendTestSamples(device, deviceIndex, successes_queue):
    global test_samples_amount, test_mountains
...
if experiment != None:
    figname = f"mountains_log/fig/SS{samples_per_device}-BS{batch_size}-LR{learningRate}-M{momentum}-HL{size_hidden_nodes}-TT{train_time}-{experiment}.png"
    plt.savefig(figname, format='png')
    print(f"Generated {figname}")
    with open(f"mountains_log/json/SS{samples_per_device}-BS{batch_size}-LR{learningRate}-M{momentum}-HL{size_hidden_nodes}-TT{train_time}-{experiment}-"+str(datetime.datetime.now()).replace(":","-").replace(" ","-"),"w") as f:
